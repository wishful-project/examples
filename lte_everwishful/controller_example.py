...
controller.add_module(moduleName="discovery", pyModuleName="wishful_module_discovery_pyre",
                      ...
                      kwargs={"iface": controller_PC_interface, "groupName": group_name, "downlink": "tcp://" + controller_PC_ip_address + ":8990",
                              "uplink": "tcp://" + controller_PC_ip_address + ":8989"})
...
def setEPC(controller, node):
    ...
    #set EPC parameters
    UPIargs_EPC = {net.MME_REALM.key: 'test'}
    result = controller.node(node).radio.set_parameters(UPIargs_EPC)

    # This UPI function stops the HSS
    rvalue = controller.nodes(node).net.EPC_deactivation()
    gevent.sleep(5)
    # This UPI function starts the HSS
    rvalue = controller.nodes(node).net.EPC_activation()
    gevent.sleep(5)
...
def setENB(controller, node):
    ...
    result = controller.node(node).radio.set_parameters(UPIargs_eNB)

    # This UPI function stops the eNB, if present on node#
    rvalue = controller.nodes(node).net.eNB_deactivation()

    ##This UPI function starts the eNB
    rvalue = controller.nodes(node).net.eNB_activation()
