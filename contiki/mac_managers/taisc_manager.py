...
class LocalTAISCMACManager(LocalMACManager):
    ...
    def __init__(self, control_engine, mac_mode):
        super(LocalTAISCMACManager, self).__init__(control_engine, mac_mode)
        pass
    ...
    def add_slots(self, slotframe_csv):
        ...
        current_offset = 0
        ret_val = 0
        taisc_mac_address_slot_list_dict = read_taisc_slot_list_local(slotframe_csv, self.get_hwaddr_list())
    ...
    def blacklist_channels(self, channel_lst):
        ...
        if self.mac_mode == "TSCH":
            ...
            for channel in hopping_sequence.TAISCHoppingSequence:
...
class GlobalTAISCMACManager(GlobalMACManager):
    ...
    def __init__(self, control_engine, mac_mode):
        super(GlobalTAISCMACManager, self).__init__(control_engine, mac_mode)
        pass
    ...
    def add_slots(self, slotframe_csv):
        current_offset = 0
        ret_val = 0
        taisc_mac_address_slot_list_dict = read_taisc_slot_list_global(slotframe_csv, self.get_hwaddr_list())
        for mac_address in taisc_mac_address_slot_list_dict.keys():
            taisc_slotlist = taisc_mac_address_slot_list_dict[mac_address]
            while(current_offset < taisc_slotlist.slot_list_length):
                slotlist_tpl = taisc_slotlist.to_tuple(current_offset, MAX_MSG_SIZE)
...
class TAISCHoppingSequence(object):
    ...
    def __init__(self, hopping_sequence_list):
        ...
        Args:
            TSCHHoppingSequenceList (list): a hopping sequence containing 802.15.4 channels.
        """
        self.hopping_sequence_list = hopping_sequence_list
    ...
    def to_tuple(self):
        ...
        """
        return tuple(self.hopping_sequence_list)
    ...
    def __str__(self):
        return str(self.hopping_sequence_list)
...
def read_taisc_slot_list_local(slot_list_csv, mac_adress_list):
...
def read_taisc_slot_list_global(slot_list_csv, mac_adress_list):
    """Create TSCH slotlist from CSV file.
    """
    try:
        file_sf = open(slot_list_csv, 'rt')
