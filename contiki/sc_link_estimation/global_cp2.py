...
if __name__ == "__main__":
    ...
    try:
        ...
        for i in range(1, len(radio_platforms)):
            ...
            print(ret)
            gevent.sleep(5)
            val = controller.blocking(True).node(nodes[0]).net.iface("lowpan0").get_measurements_net(['udp_stats'])
            control_msg_rx_overhead["lowpan0"] += 2
            control_msg_tx_overhead["lowpan0"] += 2
            num_rx = val['udp_stats'][1] - control_msg_rx_overhead["lowpan0"]
            print(num_rx - prev_num_rx)
            prev_num_rx = num_rx
