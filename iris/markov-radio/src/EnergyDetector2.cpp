...
void EnergyDetector2::set_parameters(uint16_t _ma_step_size, uint16_t fftsize, uint16_t num_channels, float bin_mask_select_perc, float bin_mask_ref_perc, double second_av_size) 
{
    mavg_size = 160;
    mavg_step_size = _ma_step_size;
    Nch = num_channels;
    nBins = fftsize;
    second_ave_size = second_av_size;
    // Define the bin mask
...
void EnergyDetector2::setup() 
{
    ...
    ch_pwr_ma_last_outputs.resize(Nch, MovingWindowMax(mavg_size*second_ave_size)); //1.75  // previous moving average outputs to compensate 50% duty cycle
