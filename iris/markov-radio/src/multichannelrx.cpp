             ...
             void *           _userdata)
{
    ...
    if (_header_valid) {
        ...
        if (_payload_valid) {
           ...
           // std::cout << "the payload is : ";
           // std::cout << "channel no is : " << _header[4] << std::endl;
            //std::cout << "header 5 is : " << _header[5] << std::endl;
          //  for(int i = 0; i < _payload_len;i++)
          //      std::cout << *(_payload+i);
                        
         //   std::cout << std::endl;
            
            // pass received frame to challenge DBz
            std::cout << "the modulation order is :" <<  (int)_header[6]<< std::endl;
              std::cout << "the modulation order is :" <<  _stats.mod_scheme << std::endl;
            if(_header[5] == 'm')
            {
                
               
                    mod_totals[(int)_header[6]] = mod_totals[(int)_header[6]] + _payload_len;
            }
            if (params_.use_db) {
...
multichannelrx::~multichannelrx()
{
    ...
    std::cout << "channel 1 packets : " << channel1packets << std::endl;
    std::cout << "channel 2 packets : " << channel2packets << std::endl;
    std::cout << "channel 3 packets : " << channel3packets << std::endl;
    std::cout << "channel 4 packets : " << channel4packets << std::endl;
    
    int temp = 0;
    int temp_ind;
    for(int i =0; i < mod_totals.size();i++)
    {
        if(mod_totals[i] > temp)
        {
            temp = mod_totals[i];
            temp_ind = i;
        }
    ...
    std::cout << "best throughput is from mod order  :" << temp_ind << std::endl;
    if (total_frames > 0) {
