...
/**
 * This a frequency domain energy detector that is uses the guard bands for online noise floor estimation
 */
class EnergyDetector2
{
    fftwf_plan fft;                        ///< Our FFT object pointer.
    uint16_t bin_idx;
    double second_ave_size;
    uint16_t Nch;
    uint16_t mavg_size;
...
public:
    ...
    void set_parameters(uint16_t _avg_win_size = 16, uint16_t fftsize = 512, uint16_t num_channels = 4, float bin_mask_select_perc = 0.5, float bin_mask_ref_perc = 0.5,double second_ave_size = 1.75);
    void setup();
    void destroy();
