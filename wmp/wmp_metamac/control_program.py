...
def my_local_control_program():
	...
	def queue_multipush(story_channel, story_file, story_channel_len, story_channel_len_diff):
		...
		# 	# 	  ( int(slots[ai].slot_num), int(slots[ai].read_num), int(slots[ai].host_time),
		# 	# 		slots[ai].tsf_time, slots[ai].slot_index, slots[ai].slots_passed,
		# 	# 		ord(slots[ai].filler), ord(slots[ai].packet_queued), ord(slots[ai].transmitted),
		# 	# 		ord(slots[ai].transmit_success), ord(slots[ai].transmit_other),
		# 	# 		ord(slots[ai].bad_reception), ord(slots[ai].busy_slot), ord(slots[ai].channel_busy) ))
		# 	#save on file
		...
		# 			ord(slots[ai].bad_reception), ord(slots[ai].busy_slot), ord(slots[ai].channel_busy) ))
		# 	ai += 1
		# 	#print("story_channel_len %d" % len(story_channel))
		...
		while ai < story_channel_len :
			...
			ord(story_channel[ai].filler), ord(story_channel[ai].packet_queued), ord(story_channel[ai].transmitted),
			ord(story_channel[ai].transmit_success), ord(story_channel[ai].transmit_other),
			ord(story_channel[ai].bad_reception), ord(story_channel[ai].busy_slot), ord(story_channel[ai].channel_busy) ))
	...
	def update_weights(suite, current_slot):
		...
		if (current_slot.packet_queued) :
			...
			for p in range(suite.num_protocols) :
				...
				# 		current_slot.slot_num, suite->slot_offset, suite->last_slot);
			 	...
			 	#d = tdma_emulate(suite.protocols[p].parameter, current_slot.slot_num, suite.slot_offset, suite.last_slot)
	...
	protocols[0].id = 1
	protocols[0].name = 'TDMA  (slot 0)'
	protocols[0].fsm_path = 'tdma-4.txt'
	#protocols[0].fsm_params = '/params'
	protocols[0].fsm_params[0].num = 12
	protocols[0].fsm_params[0].val = 4
	protocols[0].fsm_params[1].num = 11
	protocols[0].fsm_params[1].val = 0
	protocols[0].emulator = 'tdma'
	...
	protocols[1].id = 2
	protocols[1].name = 'TDMA  (slot 1)'
	protocols[1].fsm_path = 'tdma-4.txt'
	#protocols[0].fsm_params = '/params'
	protocols[1].fsm_params[0].num = 12
	protocols[1].fsm_params[0].val = 4
	protocols[1].fsm_params[1].num = 11
	protocols[1].fsm_params[1].val = 1
	protocols[1].emulator = 'tdma'
	#protocols[0].parameter = 'params';
	...
	protocols[2].id = 3
	protocols[2].name = 'TDMA  (slot 3)'
	protocols[2].fsm_path = 'tdma-4.txt'
	#protocols[0].fsm_params = '/params'
	protocols[2].fsm_params[0].num = 12
	protocols[2].fsm_params[0].val = 4
	protocols[2].fsm_params[1].num = 11
	protocols[2].fsm_params[1].val = 2
	protocols[2].emulator = 'tdma'
	#protocols[0].parameter = 'params';
	...
	protocols[3].id = 4
	protocols[3].name = 'TDMA  (slot 4)'
	protocols[3].fsm_path = 'tdma-4.txt'
	#protocols[0].fsm_params = '/params'
	protocols[3].fsm_params[0].num = 12
	protocols[3].fsm_params[0].val = 4
	protocols[3].fsm_params[1].num = 11
	protocols[3].fsm_params[1].val = 3
	protocols[3].emulator = 'tdma'
	#protocols[0].parameter = 'params';
	...
	t = threading.Thread(target=acquire_slots_channel, args=(story_channel,))
	t.start()
	...
	while True: #(metamac_loop_break == 0)
		...
		print('\n\nstory_channel len %d - diff %d' % (story_channel_len, story_channel_len_diff))
		if story_channel_len_diff > 0 :
			...
			for i in range((story_channel_len - story_channel_len_diff), story_channel_len ) :
				update_weights(suite, story_channel[i])
